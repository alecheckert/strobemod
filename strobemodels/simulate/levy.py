...
from strobemodels.simulate.simutils import sample_sphere
...
class LevyFlight3D(object):
    ...
    ------------------
        ...
        L = LevyFlight3D(40.0, alpha=2.0, D=2.0, dt=0.01, method="radon")
    ...
    def cf(self, k=None):
        ...
        if k is None:
            k = self.k
        return np.exp(-self.D * self.dt * np.power(np.abs(k), self.alpha))
    ...
    def _gen_pdf_rad(self):
        ...
        if self.method == "radon":
            arg = self.D * self.dt * np.power(np.abs(self.k), self.alpha)
